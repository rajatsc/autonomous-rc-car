...
class MPPIController:
  ...
  def __init__(self, T, K, sigma=[0.5, 0.1], _lambda=0.5, testing=False):
    ...
    model_name = rospy.get_param("~nn_model", "/media/JetsonSSD/model_residual.torch")
    self.res_model = torch.load(model_name)
    self.res_model.cuda() # tell torch to run the network on the GPU
    self.res_model.eval() # Model ideally runs faster in eval mode
  ...
  def cost(self, pose, goal, ctrl, noise, t):
    ...
    if mean_dist < 0.4 and mean_dist > 0.2:
      theta_weight = 5.0
      pose_cost = 1.0 / (5 * euclidian_distance) + theta_distance * theta_weight * (1.2 ** t)# (K,)
    elif mean_dist < 0.2:
      theta_weight = 4.8
      eucl_weight = 1.0
      pose_cost = euclidian_distance * eucl_weight + theta_distance * theta_weight / (t+1) # (K,)
    else:
      theta_weight = 2.8
      eucl_weight = 1.0
      pose_cost = euclidian_distance * eucl_weight + theta_distance * theta_weight / (t+1) # (K,)
  ...
  def mppi(self, init_pose, init_input):
    ...
    for t in xrange(self.T):
      ...
      residual = self.res_model(Variable(nn_input, requires_grad=False)).data # (K, 3)
      pose = pose - residual #(K, 3)
      ...
      delta_x, delta_y, delta_theta = self.model.ackerman_equations()

      pose_dot[:,0] = delta_x - residual[:,0] #(K,)
      pose_dot[:,1] = delta_y - residual[:,1] #(K,)
      pose_dot[:,2] = delta_theta - residual[:,2] #(K,)
...
if __name__ == '__main__':
  ...
  T = 21
  K = 800
  sigma = [0.2, 0.4] # TODO: These values will need to be tuned
  _lambda = 0.1
