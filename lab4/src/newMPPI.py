...
class MPPIController:
  ...
  def __init__(self, T, K, sigma=[0.5, 0.1], _lambda=0.5, testing=False):
    ...
    self.cur_pose = torch.Tensor(self.K, 3).type(self.dtype) # (K,3)
    ...
    self.dist_to_goal = torch.Tensor(self.K, 3).type(self.dtype)
  ...
  def cost(self, pose, goal, ctrl, noise, t):
    ...
    self.ctrl_cost.copy_(ctrl).mul_(self._lambda).mul_(self.inv_sigma).mul_(noise).mul_(1.9)
    self.ctrl_cost.abs_().sum(dim=1, out=self.running_cost) # (K,) copied into running_cost

    ## THIRD - do a bounds check
    Utils.world_to_map_torch(self.cur_pose, self.map_info, self.map_angle, self.map_c, self.map_s) # cur pose is now in pixels
   ...
   # ys *= (~bad_ks).float() # (K,) modified bad coords to be in range, so we can index the map
    ...
    self.bounds_check.copy_(self.permissible_region[ys.long(), xs.long()]) # (K,) with map value 0 or 1
  ...
  def mppi(self, init_pose, init_input):
    ...
    for t in xrange(self.T):
      ...
      pose_dot.copy_(self.model(Variable(nn_input, requires_grad=False)).data) # (K, 3)
    ...
    weights = weights.unsqueeze(1).expand(T,K,2) # (T,K,2)
    weights.mul_(self.noise)
    weights.sum(dim=1, out=self.ctrl_change) # (T,2)
