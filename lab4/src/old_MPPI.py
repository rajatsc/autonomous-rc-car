...
class MPPIController:
  ...
  def __init__(self, T, K, sigma=[0.5, 0.1], _lambda=0.5, testing=False):
    ...
    self.XY_THRESHOLD = float(rospy.get_param("/threshold/goal/dist", 0.25))
    ...
    self.pose = torch.Tensor(self.K,3).type(self.dtype) #(K,3)
  ...
  def cost(self, pose, goal, ctrl, noise, t):
    # input shapes: (K,3), (3,), (2,), (K,2), (float)
    ...
    euclidian_distance = dist_to_goal[:,:2].norm(p=2, dim=1).pow_(2)  # (K,)

    theta_distance = dist_to_goal[:,2]
    Utils.clamp_angle_tensor_(theta_distance)
    theta_distance.abs_()
    self.pose_cost = euclidian_distance.mul_(1.7).add_(theta_distance.mul_( 0.5 * (1.2 ** (t * 0.5))))
    ...
    ctrl_cost = ctrl_cost.abs_().sum(dim=1) # (K,)
   ...
   # ys *= (~bad_ks).float() # (K,) modified bad coords to be in range, so we can index the map
    ...
    BOUNDS_COST = 10000
    bounds_check = self.bad_ks.mul_(BOUNDS_COST)
  ...
  def mppi(self, init_pose, init_input):
    ...
    self.pose.copy_(init_pose.repeat(self.K, 1)) # pose (K, 3)

    # create one input for each trajectory
    init_input[2] = Utils.clamp_angle(init_input[2])
    self.nn_input.copy_(init_input.repeat(self.K, 1)) # nn_input (K, 8)
    ...
    for t in xrange(self.T):
      ...
      self.nn_input[:,5:7].add_(self.ctrl[t]).add_(self.noise[t]) # (2,) + (K,2)
      ...
      self.pose.add_(pose_dot) # Update pose
      ...
      self.cost(self.pose, self.goal_tensor, self.ctrl[t], self.noise[t], t) # (K,3), (3,), (2,), (K,2) => (K,)
  ...
  def mppi_cb(self, msg):
    ...
    self.curr_pose = torch.Tensor([msg.pose.position.x,
                              msg.pose.position.y,
                              theta]).type(self.dtype)

    difference_from_goal = np.sqrt((self.curr_pose[0] - self.goal_tensor[0])**2 + 
                                   (self.curr_pose[1] - self.goal_tensor[1])**2) 
    if difference_from_goal < 0.4:
      self.MIN_VEL = -0.75 #TODO make sure these are right
      self.MAX_VEL = 0.75
   # else:
   #   self.MIN_VEL = -0.75 #TODO make sure these are right
   #   self.MAX_VEL = 0.75
    ...
    pose_dot = self.curr_pose - self.last_pose # get state
    self.last_pose = self.curr_pose

    timenow = msg.header.stamp.to_sec()
    dt = timenow - self.lasttime
    self.lasttime = timenow
    self.init_input = torch.Tensor([pose_dot[0], pose_dot[1], pose_dot[2],
                             np.sin(theta), np.cos(theta),
                             0.0, 0.0, dt]).type(self.dtype)

    self.pose_pub.publish(Utils.particle_to_posestamped(self.curr_pose, 'map'))

    #poses = self.mppi(curr_pose, init_input)
    ...
    if self.viz and self.viz_poses is not None:
      self.visualize(self.viz_poses)

    # For testing: send control into model and pretend this is the real location
    if self.testing:
      test_nn_input = init_input.clone()
      ...
      test_pose = self.curr_pose + test_pose_dot
...
if __name__ == '__main__':
  ...
  T = 25
  K = 2000
  sigma = [0.35, 0.15] # TODO: These values will need to be tuned
  _lambda = .01
  ...
  while not rospy.is_shutdown():
   # mp.check_at_goal()
    if mp.curr_pose is not None and mp.init_input is not None:
      mp.viz_poses = mp.mppi(mp.curr_pose, mp.init_input)
    mp.send_controls()
    #rate.sleep()
